...
{
//printf("in AIS_KfifoMin\n");
//printf("exit AIS_KfifoMin\n");
    return (x1 < x2) ? x1 : x2;
}

unsigned int AIS_KfifoGet(AisKfifo *pstKFifo, void **element)
{
//printf("in AIS_KfifoGet\n");
  if (pstKFifo->in == pstKFifo->out)
  {
//printf("exit AIS_KfifoGet\n");
    return VOS_ERROR;
  }
...
{
//printf("in AISiLossLessLocalMsgHandler\n");
//printf("exit AISiLossLessLocalMsgHandler\n");
	return (msg == NULL) ? VOS_ERROR : VOS_OK;
}

void AIS_AppEnvProcEvent(AisAppEnv *env, unsigned char ctrlMsg)
{
//printf("in AIS_AppEnvProcEvent\n");
    void *message = NULL;
...
//printf("in AIS_AppEnvProcEpollEvent\n");
    ...
    if (!in)
    {
//printf("exit AIS_AppEnvProcEpollEvent\n");
        return;
    }
