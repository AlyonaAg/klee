...
uint32_t my_fei_hstm_check_ilk_get_tmid_and_ilkid(union tagFEI_HSTM_INST_INFO fe_id_, union tagFEI_HSTM_INST_INFO fe_ilk_id_, uint32_t *tm_id, uint32_t *tm_ilk_id)
{
  uint32_t fe_id = fe_id_.stInstBitInfo.uiChipId;
  uint32_t fe_ilk_id = fe_ilk_id_.stInstBitInfo.uiPortType;

    if (fe_id_.stInstBitInfo.uiChipId > 32) {
      printf("1\n");
        *tm_id = fe_id_.stInstBitInfo.uiChipId<<1; 
        *tm_ilk_id = fe_ilk_id_.stInstBitInfo.uiPortType<<1;
    } else if ((fe_id_.stInstBitInfo.uiChipId <= 32) && (fe_id_.stInstBitInfo.uiChipId > 16)){
      printf("2\n");
        *tm_id = fe_id_.stInstBitInfo.uiChipId * 2; 
        *tm_ilk_id = fe_ilk_id_.stInstBitInfo.uiPortType;
    } else {
      printf("3\n");
        *tm_id = fe_id_.stInstBitInfo.uiChipId; 
        *tm_ilk_id = fe_ilk_id_.stInstBitInfo.uiPortType * 2;
    }
    return 0;
}
...
void hstm_dfx_ilk_lane_isolate_health_info(union tagFEI_HSTM_INST_INFO bit_info, unsigned int lane_id, unsigned int nof_lane)
{
    ...
    printf("!\n");
		tm_id += lane_id;
		core_id += nof_lane;
		    (void)my_fei_hstm_check_ilk_get_tmid_and_ilkid(bit_info, 
        bit_info, &tm_id, &core_id);
	} else if (nof_lane > 16){
    printf("?\n");
		tm_id = lane_id>>1;
